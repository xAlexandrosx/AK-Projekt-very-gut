...
int main(void)
{
  ...
  while (1)
  {
	  ...
	  button_state = HAL_GPIO_ReadPin(GPIOC, GPIO_PIN13);

	  if (button_last == GPIO_PIN_SET && button_state == GPIO_PIN_RESET) {
	  ...
	  if (HAL_UART_Recieve(&huart2, (uint8_t*)&current, 1, 10) = HAL_OK) {
  ...
  for (int i = 0; i < n; i++)
  {
	  ...
	  int random_number = rand() % (10000 - 1000 + 1) + 1000;
	  int t = 0;
	  int guzior_wcisniety = 0;
	  ...
	  // Biały ekran, liczenie czasu od teraz
	  ssd1306_Fill(White);
	  while (guzior_wcisniety != 1)
	  {
		  // Wciśnięcie guzika, koniec liczenia czasu reakcji
			if(HAL_GPIO_ReadPin(B1_GPIO_Port, B1_Pin) == GPIO_PIN_RESET)
			{

				fsprint("Czas reakcji: %c ms \r\n", bufor);
				HAL_UART_Transmit(&huart2, (uint_8 *)bufor, 64, 1000);
				guzior_wcisniety = 1;
			}



		  // Liczenie co milisekunde
		  HAL_Delay(1);
		  t++;
	  }
	  guzior_wcisniety = 0;
